...
Le Heltec se connecte au **Wi-Fi** et reste totalement autonome : le Pi s'y connecte en TCP/IP comme n'importe quel client réseau, et vous pouvez **en parallèle** vous y connecter en Bluetooth (BLE) avec votre cellulaire et l'appli Meshtastic — Wi-Fi et BLE fonctionnent indépendamment et simultanément. Aucun câble, aucune dépendance au Pi.
...
```
              ...
              ▼                                   ▼
┌──────────────────────────────────┐    ┌────────────────────┐
│         Raspberry Pi 5           │    │   Votre cellulaire  │
│                                   │    │  (appli Meshtastic) │
│  ┌─────────────┐ ┌────────────┐  │    └────────────────────┘
...
### 3.3 Configuration requise du nœud
...
# Position fixe (remplacez par vos coordonnées réelles)
meshtastic --port /dev/ttyACM0 --setlat 45.5678 --setlon -73.1234 --setalt 150

# Activer le Wi-Fi sur le nœud
meshtastic --port /dev/ttyACM0 --set wifi.enabled true
meshtastic --port /dev/ttyACM0 --set wifi.ssid "VotreReseauWiFi"
meshtastic --port /dev/ttyACM0 --set wifi.psk "VotreMotDePasseWiFi"
```

Le nœud va redémarrer et se connecter au Wi-Fi. Le Bluetooth (BLE) reste actif par défaut en parallèle — c'est ce qui vous permettra de vous y connecter avec l'appli Meshtastic sur votre cellulaire, indépendamment du Pi.

### 3.4 Trouver l'adresse IP du Heltec

Une fois le Wi-Fi connecté, trouvez son IP :
- Sur l'écran du Heltec (l'IP s'affiche généralement à l'écran d'accueil)
- Via l'appli Meshtastic sur votre téléphone (connectée en BLE, l'IP Wi-Fi est affichée dans les infos du nœud)
- Scan réseau : `nmap -sn 192.168.1.0/24`

**Fortement recommandé :** réservez une **IP statique** pour le Heltec sur votre routeur (réservation DHCP par adresse MAC), pour qu'elle ne change jamais et que MeshMonitor ne perde pas la connexion après un redémarrage du nœud.

Notez cette IP (ex : `192.168.1.150`) — c'est elle qu'on utilise dans le docker-compose à la Partie 4.

### Dépannage Wi-Fi

| Symptôme | Solution |
|---|---|
| Le nœud ne se connecte pas au Wi-Fi | Vérifiez le SSID/mot de passe : `meshtastic --port /dev/ttyACM0 --get wifi` |
| L'IP change après chaque reboot | Réservez une IP statique par MAC sur votre routeur |
| MeshMonitor ne se connecte plus au nœud | `ping 192.168.1.150` puis `telnet 192.168.1.150 4403` pour tester le port TCP |
...
2. Le Heltec étant maintenant en **Wi-Fi** (Partie 3) et non branché en USB au Pi, **il n'y a plus de conteneur `serial-bridge`** — MeshMonitor se connecte directement à l'IP du nœud sur le réseau, exactement comme n'importe quel appareil.
...
services:
  meshmonitor:
    ...
    environment:
      ...
      # Remplacez par l'IP Wi-Fi réelle de votre Heltec (Partie 3.4) — idéalement une IP statique/réservée
      - MESHTASTIC_NODE_IP=192.168.1.150
      - ALLOWED_ORIGINS=http://localhost:8080,http://meshstation.local:8080
      - TZ=America/Toronto
...
**Notes :**
- Remplacez `192.168.1.150` par l'IP Wi-Fi réelle de votre Heltec
- `ALLOWED_ORIGINS` doit inclure toutes les façons dont vous accédez à l'interface (localhost, hostname, IP)
...
| Kiwix (Wikipedia) | http://meshstation.local:8082 | 8082 |
| Heltec (Wi-Fi, TCP Meshtastic) | 192.168.1.150 *(votre IP réelle)* | 4403 |
...
4. Envisager un point d'accès Wi-Fi sur le Pi pour que d'autres appareils accèdent à MeshMonitor et Kiwix sur le terrain
